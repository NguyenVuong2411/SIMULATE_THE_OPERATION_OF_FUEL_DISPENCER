 ...
 * Darren Rook          11/01/14    Added TICK_ISR_POLLING for not
 *                            using an ISR.  Only works on PCH and PCD.
********************************************************************/
#define __TICK_C
...
/*****************************************************************************
  ...
  ***************************************************************************/
#if getenv("BIT_VALID:TMR0IP")
#bit _TMR0IP=getenv("BIT:TMR0IP")
#endif
void TickInit(void)
{
#if defined(__18CXX)
...
/*****************************************************************************
  ...
  Returns:
     None
  ***************************************************************************/
#if defined(__PCD__) && defined(TICK_ISR_POLLING) //__CCS__ __PCH__ __PCD__ ccs added
...
static void GetTickCopy(void)
{
   ...
   } while (interrupt_active(INT_TIMER0));
   #if !defined(TICK_ISR_POLLING)
   enable_interrupts(INT_TIMER0);
   #endif
...
/*****************************************************************************
  ...
  ***************************************************************************/  
#if defined(__DEBUG_PUSH_TICKGET__)
DWORD _TickGet(void)
#else
DWORD TickGet(void)
#endif
